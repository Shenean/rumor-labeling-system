<template>
  <div class="review-event-detail">
    <el-card>
      <template #header>
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <span>事件审核 #{{ eventId }}</span>
...
const fetchDetail = async () => {
  loading.value = true
  try {
    const res: any = await api.get(`/review/events/${eventId.value}`)
...
const submit = async (approved: boolean) => {
  ...
  try {
    await api.post(`/review/events/${eventId.value}`, { approved, comments: comments.value })
